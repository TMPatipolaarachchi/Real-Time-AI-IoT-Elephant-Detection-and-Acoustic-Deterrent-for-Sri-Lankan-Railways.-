...
class NotificationSyncService {
  ...
  initializeNetworkMonitoring() {
    NetInfo.addEventListener(state => {
      ...
      console.log('[NotificationSync] Network status:', this.isOnline ? 'ONLINE' : 'OFFLINE');
      
      // Auto-sync when coming back online
      if (!wasOnline && this.isOnline) {
        console.log('[NotificationSync] Back online - starting auto-sync...');
  ...
  async saveNotification(notificationData) {
    try {
      ...
      console.log('[NotificationSync] Saved locally:', notification.id);

      // Try to sync immediately if online
      if (this.isOnline) {
        await this.syncSingleNotification(notification);
      } else {
        console.log('[NotificationSync] Offline - notification queued for sync');
      }

      return { success: true, notification };
    } catch (error) {
      console.error('[NotificationSync] Error saving notification:', error);
  ...
  async saveToLocalStorage(notification) {
    try {
      ...
      console.error('[NotificationSync] Error saving to local storage:', error);
      throw error;
    }
  }
  ...
  async addToSyncQueue(notificationId) {
    try {
      ...
      console.error('[NotificationSync] Error adding to sync queue:', error);
    }
  }
  ...
  async syncSingleNotification(notification) {
    try {
      ...
      console.log('[NotificationSync] Synced to Firestore:', docRef.id);
      ...
      console.error('[NotificationSync] Error syncing notification:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  async syncNotifications() {
    if (this.isSyncing) {
      console.log('[NotificationSync] Sync already in progress');
      return;
    }

    if (!this.isOnline) {
      console.log('[NotificationSync] Offline - cannot sync');
      return { success: false, error: 'Offline' };
    }
    ...
    try {
      ...
      if (queue.length === 0) {
        console.log('[NotificationSync] No notifications to sync');
        this.notifySyncListeners({ status: 'complete', synced: 0 });
        return { success: true, synced: 0 };
      }

      console.log(`[NotificationSync] Syncing ${queue.length} notifications...`);
      ...
      console.log(`[NotificationSync] Sync complete - ${syncedCount} synced, ${failedCount} failed`);
      ...
      console.error('[NotificationSync] Sync error:', error);
  ...
  async markAsSynced(localId, firestoreId) {
    try {
      ...
      console.error('[NotificationSync] Error marking as synced:', error);
    }
  }
  ...
  async removeFromSyncQueue(notificationId) {
    try {
      ...
      console.error('[NotificationSync] Error removing from queue:', error);
    }
  }
  ...
  async getLocalNotifications() {
    try {
      ...
      console.error('[NotificationSync] Error getting local notifications:', error);
      return [];
    }
  }
  ...
  async getUnsyncedCount() {
    try {
      ...
      console.error('[NotificationSync] Error getting unsynced count:', error);
      return 0;
    }
  }
  ...
  async getLastSyncTime() {
    try {
      ...
      console.error('[NotificationSync] Error getting last sync time:', error);
      return null;
    }
  }
  ...
  async clearLocalNotifications() {
    try {
      ...
      console.log('[NotificationSync] Local notifications cleared');
      return { success: true };
    } catch (error) {
      console.error('[NotificationSync] Error clearing notifications:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  async fetchFromFirestore(userId, limit = 50) {
    try {
      ...
      console.log(`[NotificationSync] Fetched ${notifications.length} notifications from Firestore`);
      
      return { success: true, notifications };
    } catch (error) {
      console.error('[NotificationSync] Error fetching from Firestore:', error);
      return { success: false, error: error.message };
    }
  }
